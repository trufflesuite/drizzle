...
class Drizzle {
  ...
  getContracts() {
    ...
    for (var i = 0; i < this.options.contracts.length; i++) {
      ...
      const events =
        contractArtifact.contractName in this.options.events
          ? this.options.events[contractArtifact.contractName]
          : []

      this.contracts[contractArtifact.contractName] = new DrizzleContract(
        contractArtifact,
        web3,
        store,
        events
