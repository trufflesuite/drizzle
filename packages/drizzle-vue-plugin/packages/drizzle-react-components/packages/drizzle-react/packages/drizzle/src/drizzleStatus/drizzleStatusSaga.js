...
function* initializeDrizzle(action) {
  try {
    ...
    for (var i = 0; i < options.contracts.length; i++) {
      ...
      action.drizzle.contracts[contractArtifact.contractName] = yield call(
        instantiateContract,
        { contractArtifact, events, store: action.drizzle.store, web3 }
      )
    }
    ...
    for (var contract in action.drizzle.contracts) {
      contractNames.push(
        action.drizzle.contracts[contract].contractArtifact.contractName
      )
      contractAddresses.push(
        action.drizzle.contracts[contract].options.address.toLowerCase()
      )
    }
